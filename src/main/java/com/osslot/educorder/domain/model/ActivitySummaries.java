...
public record ActivitySummaries(
  ...
  public record InstitutionActivities(Institution institution, Map<String, PatientActivities> patientActivities, Duration durations, int count) {

        public InstitutionActivities(Map<String, PatientActivities> patientActivities) {
        this(patientActivities.values().stream().findFirst().map(PatientActivities::patient).map(Patient::institution).orElseThrow(),
                patientActivities,
                totalDurations(patientActivities.values().stream().flatMap(patientActivities1 -> patientActivities1.activitiesByType.values().stream())),
                getCount(patientActivities.values().stream().flatMap(patientActivities1 -> patientActivities1.activitiesByType.values().stream())));
        }
  }

  public record PatientActivities(Patient patient, Map<ActivityType, Activities> activitiesByType, Duration durations, int count) {

    public PatientActivities(Map<ActivityType, Activities> activitiesByType) {
      this(activitiesByType.values().stream().findFirst().map(Activities::activities).map(List::getFirst).map(Activity::patient).orElseThrow(),
              activitiesByType,
  ...
  public record Activities(List<Activity> activities, Duration durations, int count) {
    ...
    public Activities(List<Activity> activities) {
      this(activities, activities.stream().map(Activity::duration).reduce(Duration.ZERO, Duration::plus), activities.size());
    }
