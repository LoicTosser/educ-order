...
public class GoogleDocAdiaphKilometersFilesRepository implements AdiaphKilometersFilesRepository {
    ...
    private static final DateTimeFormatter WRITE_DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("dd/MM/yyyy");
    private final GoogleDriveService googleDriveService;

    private final Docs docs;

    public GoogleDocAdiaphKilometersFilesRepository(GoogleCredentials googleCredentials, GoogleDriveService googleDriveService) throws GeneralSecurityException, IOException {
        this.googleDriveService = googleDriveService;
        NetHttpTransport transport = GoogleNetHttpTransport.newTrustedTransport();
        docs = new Docs.Builder(transport, JSON_FACTORY, googleCredentials.getCredentials(transport))
                .setApplicationName(EducOrderApplication.APPLICATION_NAME)
                .build();
    }

    @Override
    public Optional<String> createPatientFilesForMonth(int year, int month, Patient patient, List<ActivityKilometers> activityKilometersList) {
        var patientFileId = googleDriveService.createAdiaphPatientFile(patient, month, year);
        if (patientFileId.isEmpty()) {
            return Optional.empty();
        }
        initPatientFile(patientFileId.orElseThrow(), patient, month, year, activityKilometersList.stream().map(activityKilometers -> activityKilometers.distanceFrom() + activityKilometers.distanceTo()).reduce(0L, Long::sum));
        updateRows(patientFileId.orElseThrow(), activityKilometersList);
        return patientFileId;
    }

    private void initPatientFile(String patientFileId, Patient patient, int month, int year, Long totalDistance) {
        try {
    ...
    private static Request buildReplaceAllTextRequest(String textToReplace, String replaceText) {
        return new Request().setReplaceAllText(new ReplaceAllTextRequest().setContainsText(new SubstringMatchCriteria().setText(textToReplace).setMatchCase(true)).setReplaceText(replaceText));
    }
        ...
        try {
            ...
            for (var row : content) {
                if (row.containsKey("table")) {
                    ...
                    var insertRowRequest = IntStream.range(1, activitiesCount - rowCount + 2)
                            .mapToObj(i -> new Request().setInsertTableRow(
                            ...
                            .toList();
                    try {
                        var result = docs.documents().batchUpdate(patientFileId, new BatchUpdateDocumentRequest().setRequests(insertRowRequest)).execute();
                        log.info(result.toString());
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
    ...
    private void updateRow(String patientFileId, int rowIndex, ActivityKilometers activityKilometers) {
        var text = activityKilometers.activity().beginDate().format(WRITE_DATE_TIME_FORMATTER);
        updateCell(patientFileId, text, rowIndex, 0);
        var trip = activityKilometers.from().address() + " - " + activityKilometers.activity().location().address() + " - " + activityKilometers.to().address();
        ...
        updateCell(patientFileId, Long.valueOf(activityKilometers.distanceFrom() + activityKilometers.distanceTo()).toString(), rowIndex, 3);
    }
    ...
    private void updateCell(String patientFileId, String text, int rowIndex, int cellIndex) {
        try {
            ...
            var result = docs.documents().batchUpdate(patientFileId, new BatchUpdateDocumentRequest().setRequests(requests)).execute();
            log.info(result.toString());
            Thread.sleep(100);
    ...
    private Request buildInsertTextRequest(String text, int index) {
        return new Request().setInsertText(new InsertTextRequest().setText(text).setLocation(new Location().setIndex(index)));
    }
}
